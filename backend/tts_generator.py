...
def generate_ai_discussion(conversation, output_path="generated_discussion.wav"):
    ...
    :param conversation: List of lists, e.g., [["Person A", "Hello"], ["Person B", "Hi there!"]]
    ...
    for i, item in enumerate(conversation):
        if len(item) != 2:
            raise ValueError("Each item in the conversation must be a list of two elements: [speaker, text]")
        speaker, text = item
